    ...
    <div className="flex-1 p-6 space-y-6 overflow-auto">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-muted-foreground mt-1">
                  {isEn
                    ? 'Manage device identifier to prevent account bans'
                    : '管理设备标识符，防止账号关联和封禁'}
                </p>
              </div>
            </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-2xl font-bold">
                    {originalMachineId ? (isEn ? 'Backed' : '已备份') : isEn ? 'None' : '未备份'}
                  </p>
                  <p className="text-xs text-muted-foreground">
                    {isEn ? 'Original ID' : '原始机器码'}
                  </p>
                </div>
              </div>
            </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-medium text-warning">
                      {isEn ? 'Admin Required' : '需要管理员权限'}
                    </p>
                    <p className="text-sm text-warning/80">
                      {isEn
                        ? 'Start the local service as administrator, then modify the machine ID'
                        : '请以管理员权限启动本地服务或应用后，再修改机器码'}
                    </p>
                  </div>
                </div>
                <Button
          ...
          <CardContent className="space-y-4 relative z-10">
            ...
            {machineIdHistory.length > 0 && (
              <p className="text-xs text-muted-foreground flex items-center gap-1">
                ...
                {formatTime(machineIdHistory[machineIdHistory.length - 1].timestamp)}
              </p>
            )}
            ...
            {originalMachineId ? (
              <>
                ...
                <p className="text-xs text-muted-foreground flex items-center gap-1">
                  ...
                  {originalBackupTime ? formatTime(originalBackupTime) : isEn ? 'Unknown' : '未知'}
                </p>
                <div className="flex gap-2">
                  <Button
                    variant="outline"
            ...
            <div className="group p-5 rounded-xl border-2 border-dashed hover:border-primary/50 hover:bg-primary/5 transition-all space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {isEn ? 'Generate UUID format machine ID' : '一键生成 UUID 格式机器码'}
                  </p>
                </div>
              </div>
              <Button
            ...
            <div className="group p-5 rounded-xl border-2 border-dashed hover:border-primary/50 hover:bg-primary/5 transition-all space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {isEn ? 'Enter a specific machine ID' : '输入指定的机器码'}
                  </p>
                </div>
              </div>
              <input
        ...
        <CardContent className="p-0 divide-y">
          ...
          <div className="flex items-center justify-between p-5 hover:bg-muted/50 transition-colors">
            ...
            <Button
              ...
              onClick={() =>
                setMachineIdConfig({
                  autoSwitchOnAccountChange: !machineIdConfig.autoSwitchOnAccountChange
                })
              }
              className={cn(
                'min-w-[80px]',
          ...
          <div className="flex items-center justify-between p-5 hover:bg-muted/50 transition-colors">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {isEn
                    ? 'Assign unique ID per account, auto-apply on switch'
            ...
            <Button
              ...
              onClick={() =>
                setMachineIdConfig({
                  bindMachineIdToAccount: !machineIdConfig.bindMachineIdToAccount
                })
              }
              className={cn(
                'min-w-[80px]',
            ...
            <div className="flex items-center justify-between p-5 pl-16 bg-muted/30 hover:bg-muted/50 transition-colors">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {isEn
                      ? 'When off, generates new ID on each switch'
                      : '关闭时每次切换将随机生成新机器码'}
                  </p>
                </div>
              </div>
              <Button
                ...
                size="sm"
                onClick={() =>
                  setMachineIdConfig({ useBindedMachineId: !machineIdConfig.useBindedMachineId })
                }
                className={cn(
                  'min-w-[80px]',
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <Card
          ...
          onClick={() => setShowAccountBindings(true)}
        >
        ...
        >
          <CardContent className="p-5">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                <p className="font-semibold group-hover:text-primary transition-colors">
                  {isEn ? 'Change History' : '变更历史记录'}
                </p>
                <p className="text-sm text-muted-foreground">
                  {isEn
                    ? `${machineIdHistory.length} records`
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div className="relative bg-background rounded-xl shadow-2xl w-[600px] max-h-[80vh] flex flex-col animate-in fade-in zoom-in-95 duration-200">
              ...
              <div className="flex items-center justify-between px-6 py-4 border-b">
                <div className="flex items-center gap-2">
                  ...
                  <Badge variant="secondary">
                    {accounts.size} {isEn ? 'accounts' : '个账户'}
                  </Badge>
                </div>
                <Button
                  variant="ghost"
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-2">
                ...
                {accounts.size === 0 && (
                  <div className="text-center py-12">
                    ...
                    <p className="text-sm text-muted-foreground">
                      {isEn ? 'Please add accounts first' : '请先添加账户'}
                    </p>
                  </div>
                )}
                ...
                {accounts.size > 0 &&
                  ...
                  Array.from(accounts.values()).filter((account) => {
                    ...
                    <div className="text-center py-8">
                      ...
                      <p className="text-sm text-muted-foreground">
                        {isEn ? 'Try other keywords' : '尝试其他关键词'}
                      </p>
                    </div>
                  )}
              </div>
              ...
              <div className="px-6 py-3 border-t bg-muted/50 text-xs text-muted-foreground">
                {isEn
                  ? '💡 Tip: After binding, switching to this account will auto-apply the bound machine ID'
            ...
            <div className="relative bg-background rounded-xl shadow-2xl w-[550px] max-h-[80vh] flex flex-col animate-in fade-in zoom-in-95 duration-200">
              ...
              <div className="flex items-center justify-between px-6 py-4 border-b">
                <div className="flex items-center gap-2">
                  ...
                  <Badge variant="secondary">
                    {machineIdHistory.length} {isEn ? 'records' : '条'}
                  </Badge>
                </div>
                <div className="flex items-center gap-2">
                  {machineIdHistory.length > 0 && (
              ...
              <div className="flex-1 overflow-y-auto p-4">
                {machineIdHistory.length > 0 ? (
                      ...
                      <div key={entry.id} className="p-3 bg-muted rounded-lg">
                        ...
                        {entry.accountId && (
                          <p className="text-xs text-muted-foreground mt-2">
                            {isEn ? 'Account: ' : '关联账户: '}
                            {accounts.get(entry.accountId)?.nickname ||
                              accounts.get(entry.accountId)?.email ||
                              entry.accountId}
                          </p>
                        )}
                      </div>
                  ...
                  <div className="text-center py-12">
                    ...
                    <p className="text-muted-foreground">
                      {isEn ? 'No change records' : '暂无变更记录'}
                    </p>
                    <p className="text-sm text-muted-foreground">
                      {isEn ? 'Changes will be recorded automatically' : '机器码变更后将自动记录'}
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-2 p-3 rounded-lg bg-warning/10 border border-warning/20">
                ...
                <p className="text-xs text-warning">
                  {isEn
                    ? 'Changing machine ID may affect some software licenses, proceed with caution'
