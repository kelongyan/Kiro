...
export function ProxyPoolPage(): React.ReactNode {
  ...
  useEffect(() => {
    ...
    proxyPoolConfig.autoValidateIntervalMin,
    proxyPoolConfig.autoValidateConcurrency,
    proxyPool,
    validateProxiesBatch
  ])
  ...
  const handleDeleteSelected = useCallback(() => {
    ...
    if (
      !confirm(
        isEn ? `Delete ${selectedIds.size} proxies?` : `确定删除 ${selectedIds.size} 个代理？`
      )
    )
      return
  ...
  const handleRemoveDead = useCallback(() => {
    ...
    if (
      !confirm(
        isEn ? `Remove ${deadIds.length} dead proxies?` : `确定移除 ${deadIds.length} 个失效代理？`
      )
    )
      return
            ...
            <Button
              ...
              onClick={() => {
                ...
                if (
                  confirm(
                    isEn
                      ? `Unbind all ${bindingStats.boundCount} accounts?`
                    ...
                    <div key={p.id} className="p-2 border-b last:border-b-0">
                      <div className="flex items-center justify-between gap-2 text-xs mb-1">
                        <span className="font-mono truncate" title={p.url}>
                          {p.host}:{p.port}
                          {p.label && (
                            <Badge variant="outline" className="ml-1.5 h-4 text-[9px]">
                              {p.label}
                            </Badge>
                          )}
                        </span>
  ...
  return (
    ...
    >
      ...
      <span className="flex-1 font-mono truncate" title={displayUrl}>
        ...
        {proxy.label && (
          <Badge variant="outline" className="ml-2 h-4 text-[9px]">
            {proxy.label}
          </Badge>
        )}
      </span>
