...
export const AccountCard = memo(function AccountCard({
  ...
  const handleLogout = async (): Promise<void> => {
    if (
      !confirm(
        isEn
          ? 'This will clear local SSO cache and logout from Kiro. Continue?'
          : '这将清除本地 SSO 缓存并退出 Kiro 登录，是否继续？'
      )
    ) {
      return
    }
    ...
    if (result.success) {
      ...
      alert(
        isEn
          ? `Logged out successfully, cleared ${result.deletedCount} cache files`
    ...
    >
      ...
      <CardContent className="p-4 flex-1 flex flex-col gap-3 overflow-hidden">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Badge
            variant="outline"
            className="text-[10px] h-5 px-2 text-muted-foreground font-normal border-muted-foreground/30 bg-muted/30"
          >
            {account.idp}
          </Badge>
          {/* 代理绑定徽章：可点击解绑 */}
        ...
        {account.lastError && !isUnauthorized && (
          <div
            className="bg-red-50 text-red-600 text-[10px] p-1.5 rounded flex items-center gap-1.5 truncate mt-1"
