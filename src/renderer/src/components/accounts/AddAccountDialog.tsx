...
export function AddAccountDialog({ isOpen, onClose }: AddAccountDialogProps): React.ReactNode {
  ...
  const isAccountExists = useCallback(
    (email: string, userId: string, provider?: string): boolean => {
  ...
  const handleLoginSuccess = useCallback(
    ...
    [addAccount, isAccountExists, isEn, onClose, resetForm]
  )
  ...
  useEffect(() => {
    ...
    return () => unsubscribe()
  }, [handleLoginSuccess, isLoggingIn, loginType])
                ...
                <div className="space-y-4 py-2">
                  <div className="text-center mb-6">
                    ...
                    <h3 className="text-lg font-semibold">
                      {isEn ? 'Choose Login Method' : '选择登录方式'}
                    </h3>
                    <p className="text-sm text-muted-foreground mt-1">
                  ...
                  <div className="px-2">
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        >
                          <EyeOff
                            className={`w-4 h-4 ${usePrivateMode ? 'text-primary' : 'text-muted-foreground'}`}
                          />
                        </div>
                        <span
                          className={`text-sm font-medium ${usePrivateMode ? 'text-foreground' : 'text-muted-foreground'}`}
                        >
                  ...
                  <div className="flex-1">
                    ...
                    <ol className="text-xs text-primary/90 list-decimal list-inside space-y-1.5">
                      ...
                      <li>
                        ...
                        <code className="px-1 py-0.5 bg-primary/15 rounded font-mono text-[10px]">
                          x-amz-sso_authn
                        </code>{' '}
                        {isEn ? 'value' : '的值'}
                      </li>
