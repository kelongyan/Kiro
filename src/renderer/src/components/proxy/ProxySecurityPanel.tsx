...
export function ProxySecurityPanel({
  ...
  const handleRegenerateCert = useCallback(async () => {
    if (
      !confirm(
        isEn
          ? 'Regenerate self-signed certificate? You will need to re-install it on clients.'
          : '重新生成自签证书？所有客户端需要重新安装。'
      )
    )
      return
  ...
  const handleRestart = useCallback(async () => {
    if (
      !confirm(
        isEn
          ? 'Restart proxy server now? Active streams will be interrupted.'
          : '立即重启反代服务器？正在进行的流式响应会被中断。'
      )
    )
      return
                ...
                {auditEntries.length === 0 ? (
                  ...
                  auditEntries
                    ...
                    .map((entry, i) => (
                      <div
                        key={i}
                        className="text-[10px] font-mono p-1.5 rounded bg-background/50 border"
                      >
