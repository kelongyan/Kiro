...
export const accountsApi = {
  ...
  refreshAccountToken: (account: unknown): Promise<unknown> => ipcRenderer.invoke('refresh-account-token', account),
  checkAccountStatus: (account: unknown): Promise<unknown> => ipcRenderer.invoke('check-account-status', account),
  ...
  onBackgroundRefreshProgress: (callback: (data: { completed: number; total: number; success: number; failed: number }) => void): (() => void) => {
    const handler = (_event: IpcRendererEvent, data: { completed: number; total: number; success: number; failed: number }): void => callback(data)
  ...
  onBackgroundRefreshResult: (callback: (data: { id: string; success: boolean; data?: unknown; error?: string }) => void): (() => void) => {
    const handler = (_event: IpcRendererEvent, data: { id: string; success: boolean; data?: unknown; error?: string }): void => callback(data)
  ...
  switchAccount: (credentials: {
    ...
    profileArn?: string
  }): Promise<{ success: boolean; error?: string }> => ipcRenderer.invoke('switch-account', credentials),
  ...
  switchAccountCli: (credentials: {
    ...
    provider?: string
    scopes?: string[]
  }): Promise<{ success: boolean; error?: string; dbPath?: string }> => ipcRenderer.invoke('switch-account-cli', credentials),

  logoutAccount: (): Promise<{ success: boolean; deletedCount?: number; error?: string }> => ipcRenderer.invoke('logout-account'),
  exportToFile: (data: string, filename: string): Promise<boolean> => ipcRenderer.invoke('export-to-file', data, filename),
