...
export const proxyApi = {
  ...
  proxyResetRequestStats: (): Promise<{ success: boolean }> =>
    ipcRenderer.invoke('proxy-reset-request-stats'),
  proxyGetLogs: (
    count?: number
  ): Promise<
    Array<{ timestamp: string; level: string; category: string; message: string; data?: unknown }>
  > => ipcRenderer.invoke('proxy-get-logs', count),
  ...
  proxyUpdateConfig: (
    ...
    ipcRenderer.invoke('proxy-update-config', config),
  ...
  proxySelfSignedCertInfo: (): Promise<{
    ...
    altNames?: string[]
    error?: string
  }> => ipcRenderer.invoke('proxy-self-signed-cert-info'),
  proxySelfSignedCertRegenerate: (): Promise<{
    ...
    altNames?: string[]
    error?: string
  }> => ipcRenderer.invoke('proxy-self-signed-cert-regenerate'),
  proxyNeedsRestart: (): Promise<{ needsRestart: boolean }> =>
    ipcRenderer.invoke('proxy-needs-restart'),
  ...
  }> => ipcRenderer.invoke('proxy-audit-log'),
  ...
  onProxyWebhookTrigger: (
    ...
    ): void => callback(data.event, data.payload)
  ...
  proxyAddAccount: (account: {
    ...
    ipcRenderer.invoke('proxy-add-account', account),
  proxyRemoveAccount: (
    accountId: string
  ): Promise<{ success: boolean; accountCount?: number; error?: string }> =>
    ipcRenderer.invoke('proxy-remove-account', accountId),
  proxySyncAccounts: (
    accounts: Array<{
      id: string
      email?: string
  ...
  proxyGetModels: (): Promise<{
    ...
    fromCache?: boolean
  }> => ipcRenderer.invoke('proxy-get-models'),
  ...
  }> => ipcRenderer.invoke('proxy-configure-clients', input),
  accountGetModels: (
    accessToken: string,
    region?: string,
    profileArn?: string,
  ...
  openSubscriptionWindow: (url: string): Promise<{ success: boolean; error?: string }> =>
    ipcRenderer.invoke('open-subscription-window', url),
  proxySaveLogs: (
    logs: Array<{ time: string; path: string; status: number; tokens?: number }>
  ): Promise<{ success: boolean; error?: string }> => ipcRenderer.invoke('proxy-save-logs', logs),
  proxyLoadLogs: (): Promise<{
    success: boolean
    logs: Array<{ time: string; path: string; status: number; tokens?: number }>
  }> => ipcRenderer.invoke('proxy-load-logs'),
  onProxyRequest: (
  ...
  onProxyStatusChange: (
    ...
    const handler = (_event: IpcRendererEvent, status: { running: boolean; port: number }): void =>
      callback(status)
  ...
  setUsageApiType: (type: 'rest' | 'cbor'): Promise<{ success: boolean; type: string }> =>
    ipcRenderer.invoke('set-usage-api-type', type),
  ...
  setUseKProxyForApi: (enabled: boolean): Promise<{ success: boolean; enabled: boolean }> =>
    ipcRenderer.invoke('set-use-kproxy-for-api', enabled),

  setProxy: (
    enabled: boolean,
    url: string
  ): Promise<{ success: boolean; error?: string; normalizedUrl?: string }> =>
    ipcRenderer.invoke('set-proxy', enabled, url)
}
