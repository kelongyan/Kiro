...
export const kproxyApi = {
  kproxyInit: (): Promise<{
    ...
    error?: string
  }> => ipcRenderer.invoke('kproxy-init'),
  kproxyStart: (config?: {
    port?: number
    host?: string
    mitmDomains?: string[]
    ...
    ipcRenderer.invoke('kproxy-start', config),
  kproxyStop: (): Promise<{ success: boolean; error?: string }> =>
    ipcRenderer.invoke('kproxy-stop'),
  kproxyGetStatus: (): Promise<{
    running: boolean
    config: unknown
    stats: unknown
    caInfo: unknown
  }> => ipcRenderer.invoke('kproxy-get-status'),
  kproxyUpdateConfig: (config: {
    ...
    ipcRenderer.invoke('kproxy-update-config', config),
  kproxySetDeviceId: (deviceId: string): Promise<{ success: boolean; error?: string }> =>
    ipcRenderer.invoke('kproxy-set-device-id', deviceId),
  kproxyGenerateDeviceId: (): Promise<{ success: boolean; deviceId?: string }> =>
    ipcRenderer.invoke('kproxy-generate-device-id'),
  kproxyAddDeviceMapping: (mapping: {
    ...
    ipcRenderer.invoke('kproxy-add-device-mapping', mapping),
  ...
  }> => ipcRenderer.invoke('kproxy-get-device-mappings'),
  kproxySwitchToAccount: (accountId: string): Promise<{ success: boolean; error?: string }> =>
    ipcRenderer.invoke('kproxy-switch-to-account', accountId),
  kproxyGetCaCert: (): Promise<{
    ...
    fingerprint?: string
    error?: string
  }> => ipcRenderer.invoke('kproxy-get-ca-cert'),
  kproxyExportCaCert: (
    exportPath?: string
  ): Promise<{ success: boolean; path?: string; error?: string }> =>
    ipcRenderer.invoke('kproxy-export-ca-cert', exportPath),
  kproxyCheckCaCertInstalled: (): Promise<{
    success: boolean
    installed: boolean
    error?: string
  }> => ipcRenderer.invoke('kproxy-check-ca-cert-installed'),
  kproxyInstallCaCert: (): Promise<{ success: boolean; message?: string; error?: string }> =>
    ipcRenderer.invoke('kproxy-install-ca-cert'),
  kproxyUninstallCaCert: (): Promise<{ success: boolean; message?: string; error?: string }> =>
    ipcRenderer.invoke('kproxy-uninstall-ca-cert'),
  ...
  onKproxyMitm: (callback: (info: { host: string; modified: boolean }) => void): (() => void) => {
    const handler = (_event: IpcRendererEvent, info: { host: string; modified: boolean }): void =>
      callback(info)
