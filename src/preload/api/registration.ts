...
export const registrationApi = {
  registrationStartAuto: (config: {
    ...
    ipcRenderer.invoke('registration-start-auto', config),
  ...
  registrationManualPhase1: (config: {
    ...
    ipcRenderer.invoke('registration-manual-phase1', config),

  registrationManualPhase2: (
    email: string,
    fullName?: string
  ): Promise<{ success: boolean; error?: string }> =>
    ipcRenderer.invoke('registration-manual-phase2', email, fullName),
  registrationManualPhase3: (
    otp: string
  ): Promise<{ success: boolean; result?: unknown; error?: string }> =>
    ipcRenderer.invoke('registration-manual-phase3', otp),
  registrationCancel: (): Promise<{ success: boolean }> =>
    ipcRenderer.invoke('registration-cancel'),
  registrationStatus: (): Promise<{ inProgress: boolean }> =>
    ipcRenderer.invoke('registration-status'),
