...
function fetchWithAppProxy(
  ...
  return fetchWithAppProxyService(url, options, overrideProxyUrl, getNetworkAgent, safeCreateProxyAgent)
}
...
function hasStoredAccountAccessToken(
  acc: StoredAccountRecord
): acc is StoredAccountRecord & { id: string; credentials: StoredAccountCredentials & { accessToken: string } } {
    ...
    config,
    {
      ...
      onPoolEmpty: async () => {
        ...
        const proxyAccounts = Object.values(accountData.accounts)
          .filter((acc): acc is StoredAccountRecord & { id: string; credentials: StoredAccountCredentials & { accessToken: string } } =>
...
app.whenReady().then(async () => {
  ...
  ipcMain.handle('get-kiro-settings', async () => {
    try {
      return await readKiroSettingsFiles()
    } catch (error) {
  ...
  ipcMain.handle('get-kiro-available-models', async () => {
    try {
      ...
      const accountData = store.get('accountData') as { accounts?: Record<string, StoredAccountRecord> } | undefined
      if (!accountData?.accounts) return { models: [] }

      // 优先使用当前激活账号（isActive），其次使用第一个 active 且有 accessToken 的账号
      const allAccounts = Object.values(accountData.accounts).filter(hasStoredAccountAccessToken)
      const account = allAccounts.find((acc) => acc.isActive && acc.credentials?.accessToken)
...
`
  ...
  ipcMain.handle('proxy-update-api-key', (_event, id: string, updates: Partial<import('./proxy/types').ApiKey>) => {
    try {
      ...
      const { id: ignoredId, createdAt: ignoredCreatedAt, usage: ignoredUsage, ...allowedUpdates } = updates
      void ignoredId
